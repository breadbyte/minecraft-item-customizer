        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            dispatcher.register(
                    CommandManager.literal("model")
                            ...
                            .then(CommandManager.literal("hitsound")
                                    .then(CommandManager.argument("play_sound", StringArgumentType.greedyString())
                                            .suggests(SuggestionProviders.AVAILABLE_SOUNDS)
                                            .executes(ModelCommands::toggleModelSound))
                            )
                            .then(CommandManager.literal("swapsound")
                                    .then(CommandManager.argument("play_sound", StringArgumentType.greedyString())
                                            .suggests(SuggestionProviders.AVAILABLE_SOUNDS)
                                            .executes(ModelCommands::addSwappingSound))
                            )
                            .then(CommandManager.literal("critsound")
                                    .then(CommandManager.argument("play_sound", StringArgumentType.greedyString())
                                            .suggests(SuggestionProviders.AVAILABLE_SOUNDS)
                                            .executes(ModelCommands::addCritSound))
                            )
                            .then(CommandManager.literal("sweepsound")
                                    .then(CommandManager.argument("play_sound", StringArgumentType.greedyString())
                                            .suggests(SuggestionProviders.AVAILABLE_SOUNDS)
                                            .executes(ModelCommands::addSweepSound))
                            )
